...
LastEditTime: 2021-05-15 00:47:38
...
class Solution(object):
    def decodeString(self, s):
        ...
        4.当碰到']'时，stack出栈一对(left_unencoded_string,multiply)，解码字符串并与result_str中的字符串（原encoded_string）进行拼接（新encoded_string），
        ...
        for char in s:
            if 'a' <= char <= 'z' or 'A' <= char <= 'Z':
                ...
                multiply = multiply*10+char
            elif char == '[':
                stack.append((result_str, multiply))
            elif char == ']':
                left_unencoded_string, multiply = stack.pop()
                result_str = left_unencoded_string+multiply*result_str
        
        return result_str

s=Solution()
resul=Solution.decodeString("3[a2[c]]")
