...
LastEditTime: 2021-05-14 23:41:10
...
class Solution(object):
    def decodeString(self, s):
        ...
        2.当碰到字母时，直接放入encoded_string中
        3.当碰到'['时，multiply和encoded_string中的内容构成一对，即(multiply,encoded_string)作为一个整体存入辅助栈stack中，
        然后multiply和encoded_string清空为后续遍历做准备。
        注意这里multiply是上个'['到当前'['的字符串中的数字字符串，例如"3[a2[c]]"中的'a'，
        encoded_string实际是last_encoded_string,即上个'['到当前'['的字符串中的字母字符串（不是右侧括号'['和']'中的encoded_string），
        例如"3[a2[c]]"中的'a'，或者如"3[aa2[c]]"中的aa。
        4.当碰到']'时，stack出栈一对(multiply,last_result_str)，解码字符串并与result_str中的字符串（原encoded_string）进行拼接（新encoded_string），
        即result_str（新encoded_string）=multiply*result_str（原encoded_string）+last_result_str
