...
LastEditTime: 2021-05-14 23:14:35
...
class Solution(object):
    def decodeString(self, s):
        """
        辅助栈法
        result_str存由内向外剥离嵌套括号并转换的字符串（编码后），multiply为嵌套括号外的倍数计算（用于乘嵌套内的字符串）。
        ...
        3.当碰到'['时，multiply和result_str中的内容构成一对，即(multiply,result_str)作为一个整体存入辅助栈中
